...
# We need to calculate balance
# The balance is inside GroupExpensesBloc state maybe? Or we calculate from state.expenses.
# Current user id is needed. We have context.read<AuthBloc>().state probably.
# Let's see if AuthState has user.
# In `GroupDetailPage`: we have `final currentUserId = ...`

# Wait, `final currentUser = context.read<AuthBloc>().state is AuthAuthenticated ? (context.read<AuthBloc>().state as AuthAuthenticated).user.id : null;`
# Actually `group_detail_page.dart` has `final user = (context.read<AuthBloc>().state as AuthAuthenticated).user;`
# So we can calculate net balance:
                                ...
                                if (state is GroupExpensesLoading) {
                                  ...
                                  if (currentUser != null) {
                                    for (var exp in state.expenses) {
                                      ...
                                      final userSplit = exp.splits[currentUser.id];
                                      if (userSplit != null) {
                                        netBalance -= userSplit;
                                      }
                                    }
                                  }
