...
class MockAccountListBloc extends MockBloc<AccountListEvent, AccountListState> implements AccountListBloc {}
...
void main() {
  ...
  group('AccountListPage', () {
    testWidgets('shows loading indicator', (tester) async {
      whenListen(mockBloc, Stream.fromIterable([const AccountListLoading()]), initialState: const AccountListLoading());
      await pumpWidgetWithProviders(tester: tester, widget: const AccountListPage());
      expect(find.byType(CircularProgressIndicator), findsOneWidget);
    });

    testWidgets('shows empty state', (tester) async {
      whenListen(mockBloc, Stream.fromIterable([const AccountListLoaded([])]), initialState: const AccountListLoaded([]));
      when(() => mockGoRouter.pushNamed(RouteNames.addAccount)).thenAnswer((_) async => {});
      await pumpWidgetWithProviders(tester: tester, router: mockGoRouter, widget: const AccountListPage());

      expect(find.text('No accounts yet'), findsOneWidget);
      await tester.tap(find.byKey(const ValueKey('button_accountList_addFirst')));
      verify(() => mockGoRouter.pushNamed(RouteNames.addAccount)).called(1);
    });

    testWidgets('shows error state and retries', (tester) async {
      whenListen(mockBloc, Stream.fromIterable([const AccountListError('Failed')]), initialState: const AccountListError('Failed'));
      await pumpWidgetWithProviders(tester: tester, widget: const AccountListPage());
    ...
    testWidgets('renders a list of AccountCards', (tester) async {
      whenListen(mockBloc, Stream.fromIterable([AccountListLoaded(mockAccounts)]), initialState: AccountListLoaded(mockAccounts));
      await pumpWidgetWithProviders(tester: tester, widget: const AccountListPage());
