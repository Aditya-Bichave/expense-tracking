...
void main() {
  ...
  group('CategorizationStatusWidget', () {
    testWidgets('renders "needsReview" state and buttons correctly', (
      ...
      final tx = mockTransaction.copyWith(
        status: CategorizationStatus.needsReview,
      );
      await pumpWidgetWithProviders(
        tester: tester,
        widget: buildTestWidget(tx),
      );

      expect(find.text('Suggest: Suggested'), findsOneWidget);
      expect(
        find.byKey(const ValueKey('button_categorization_confirm')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey('button_categorization_change')),
        findsOneWidget,
      );
    });
    ...
    testWidgets('renders "uncategorized" state and button correctly', (
      ...
      expect(
        find.byKey(const ValueKey('button_categorization_categorize')),
        findsOneWidget,
      );
    });
    ...
    testWidgets('"Confirm" button calls onUserCategorized', (tester) async {
      when(
        () => mockCallbacks.onUserCategorized(any(), any()),
      ).thenAnswer((_) {});
      final tx = mockTransaction.copyWith(
        status: CategorizationStatus.needsReview,
      );
    ...
    testWidgets(
      ...
      (tester) async {
        when(
          () => mockCallbacks.onChangeCategoryRequest(any()),
        ).thenAnswer((_) {});

        // Test "Change" button
        final txReview = mockTransaction.copyWith(
          status: CategorizationStatus.needsReview,
        );
        await pumpWidgetWithProviders(
          tester: tester,
          widget: buildTestWidget(txReview),
        );
        await tester.tap(
          find.byKey(const ValueKey('button_categorization_change')),
        );
        ...
        final txUncat = mockTransaction.copyWith(
          status: CategorizationStatus.uncategorized,
        );
        await pumpWidgetWithProviders(
          tester: tester,
          widget: buildTestWidget(txUncat),
        );
        await tester.tap(
          find.byKey(const ValueKey('button_categorization_categorize')),
        );
