...
void main() {
  group('AssetDistributionPieChart', () {
    testWidgets('renders empty state message when no positive balances exist', (tester) async {
      await pumpWidgetWithProviders(
        tester: tester,
        widget: const AssetDistributionPieChart(accountBalances: {'Bank': 0, 'Cash': -10}),
      );
      expect(find.text('No positive asset balances to chart.'), findsOneWidget);
    });

    testWidgets('renders PieChart and legends when positive balances exist', (tester) async {
    ...
    testWidgets('renders nothing when UI mode is Quantum', (tester) async {
      await pumpWidgetWithProviders(
        ...
        widget: const AssetDistributionPieChart(accountBalances: {'Bank': 1000}),
      );
      expect(find.byType(PieChart), findsNothing);
      expect(find.byType(Card), findsNothing);
    });

    testWidgets('tapping a chart section updates the touchedIndex', (tester) async {
      final data = {'Bank': 1000.0, 'Stocks': 500.0};
      await pumpWidgetWithProviders(
        tester: tester,
        widget: AssetDistributionPieChart(accountBalances: data),
      ...
      var initialRadius = (pieChart.data.sections[0] as PieChartSectionData).radius;
      expect(initialRadius, 60.0);

      // Simulate a touch event on the first section (index 0)
      final state = tester.state<AssetDistributionPieChartState>(find.byType(AssetDistributionPieChart));
      ...
      var touchedRadius = (pieChart.data.sections[0] as PieChartSectionData).radius;
  ...
  group('AssetDistributionPieChartState (Unit Tests)', () {
    test('generateColorMap assigns colors correctly and sequentially', () {
      ...
      final colorMap = AssetDistributionPieChartState.generateColorMap(accounts);
      ...
      expect(colorMap['Stocks'], AssetDistributionPieChartState.colorPalette[2]);
