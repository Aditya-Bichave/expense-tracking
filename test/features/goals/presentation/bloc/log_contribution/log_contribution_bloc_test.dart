...
class FakeUpdateContributionParams extends Fake implements UpdateContributionParams {}
class FakeDeleteContributionParams extends Fake implements DeleteContributionParams {}
...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockCheckGoalAchievementUseCase(any()))
  ...
  group('LogContributionBloc', () {
    ...
    blocTest<LogContributionBloc, LogContributionState>(
      ...
      setUp: () {
        when(() => mockAddContributionUseCase(any()))
            .thenAnswer((_) async => Right(tContribution));
      },
      build: () => bloc,
      seed: () => LogContributionState(goalId: 'g1', status: LogContributionStatus.initial),
      act: (bloc) => bloc.add(SaveContribution(
        amount: 100.0,
        date: DateTime(2023, 1, 1),
        note: 'Note',
      )),
      expect: () => [
        isA<LogContributionState>().having((s) => s.status, 'status', LogContributionStatus.loading),
        isA<LogContributionState>().having((s) => s.status, 'status', LogContributionStatus.success),
      ],
      verify: (_) {
    ...
    blocTest<LogContributionBloc, LogContributionState>(
      ...
      setUp: () {
        when(() => mockUpdateContributionUseCase(any()))
            .thenAnswer((_) async => Right(tContribution));
      },
      build: () => bloc,
    ...
    blocTest<LogContributionBloc, LogContributionState>(
      ...
      setUp: () {
        when(() => mockDeleteContributionUseCase(any()))
    ...
    blocTest<LogContributionBloc, LogContributionState>(
      ...
      expect: () => [
         isA<LogContributionState>()
            ...
            .having((s) => s.errorMessage, 'message', contains('Cannot delete')),
      ],
    );
  });
}
