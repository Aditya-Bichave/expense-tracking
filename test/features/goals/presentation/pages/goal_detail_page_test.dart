...
class MockGetContributionsUseCase extends Mock implements GetContributionsForGoalUseCase {}
...
void main() {
  ...
  final mockGoal = Goal(id: '1', name: 'Test Goal', targetAmount: 1000, totalSaved: 500);
  ...
  setUp(() {
    ...
    sl.registerSingleton<GetContributionsForGoalUseCase>(mockGetContributionsUseCase);
  });
  ...
  group('GoalDetailPage', () {
    testWidgets('shows loading and then displays details', (tester) async {
      when(() => mockGoalRepository.getGoalById(any())).thenAnswer((_) async => Right(mockGoal));
      when(() => mockGetContributionsUseCase.call(any())).thenAnswer((_) async => const Right([]));
    ...
    testWidgets('tapping Archive shows dialog and dispatches event', (tester) async {
      when(() => mockGoalRepository.getGoalById(any())).thenAnswer((_) async => Right(mockGoal));
      when(() => mockGetContributionsUseCase.call(any())).thenAnswer((_) async => const Right([]));
      ...
      verify(() => mockGoalListBloc.add(const ArchiveGoal(goalId: '1'))).called(1);
    });
  });
}
