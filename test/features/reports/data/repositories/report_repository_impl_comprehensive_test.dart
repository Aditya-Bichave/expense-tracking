...
void main() {
  ...
  group('_fetchTransactions Edge Cases', () {
    test('should return Left when expense repository fails', () async {
      when(
        () => mockExpenseRepository.getExpenses(
          startDate: any(named: 'startDate'),
          endDate: any(named: 'endDate'),
        ...
        (_) async => const Left(CacheFailure('Expense fetch failed')),
      );
      when(
        () => mockIncomeRepository.getIncomes(
          startDate: any(named: 'startDate'),
      ...
      result.fold(
        (l) => expect(l.message, 'Expense fetch failed'),
        (r) => fail('should not succeed'),
      );
    });

    test('should return Left when income repository fails', () async {
      when(
        () => mockExpenseRepository.getExpenses(
          startDate: any(named: 'startDate'),
          endDate: any(named: 'endDate'),
      ...
      when(
        ...
        (_) async => const Left(CacheFailure('Income fetch failed')),
      );
      ...
      expect(result.isLeft(), true);
      result.fold(
        (l) => expect(l.message, 'Income fetch failed'),
        (r) => fail('should not succeed'),
      );
    });
  });
