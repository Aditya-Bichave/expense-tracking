...
class GroupsRepositoryImpl implements GroupsRepository {
  ...
  Future<Either<Failure, void>> refreshGroupsFromServer() async {
    try {
      ...
      // Problem: `where(...).map(...).toList()` chains iterables and allocates multiple short-lived objects.
      // Solution: Use a direct list comprehension to filter and map in a single pass.
      // Impact: Reduces GC pressure during background group sync.
      final staleGroupIds = [
  ...
  Future<void> _syncRemoteMembersForGroupById(String groupId) async {
    try {
      ...
      // Problem: `where(...).map(...).toList()` chains iterables and allocates multiple short-lived objects.
      // Solution: Use a direct list comprehension to filter and map in a single pass.
      // Impact: Reduces GC pressure during background group member sync.
