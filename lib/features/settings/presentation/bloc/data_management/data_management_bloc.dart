...
class DataManagementBloc
  ...
  Future<void> _onBackupRequested(
      BackupRequested event, Emitter<DataManagementState> emit) async {
    log.info("[DataManagementBloc] Received BackupRequested event.");
    emit(state.copyWith(
        status: DataManagementStatus.loading, clearMessage: true));
    ...
    result.fold((failure) {
      ...
      emit(state.copyWith(
          status: DataManagementStatus.success, message: successMessage));
  ...
  Future<void> _onRestoreRequested(
      RestoreRequested event, Emitter<DataManagementState> emit) async {
  ...
  Future<void> _onClearDataRequested(
      ClearDataRequested event, Emitter<DataManagementState> emit) async {
  ...
  void _onClearMessage(
      ClearDataManagementMessage event, Emitter<DataManagementState> emit) {
    ...
    emit(state.copyWith(
        status: DataManagementStatus.initial, clearMessage: true));
  }
