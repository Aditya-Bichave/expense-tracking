  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ReportFilterBloc, ReportFilterState>(
      builder: (context, state) {
        ...
        // Problem: Chaining `.where().map().toList()` or `.map().toList()` creates intermediate iterables and closures, increasing GC pressure in the build loop.
        // Solution: Use list comprehensions `[for ... if ...]` to iterate once directly into the final list.
        // Impact: Reduces object allocations during UI rebuilds, improving scroll and frame performance.
        final categoryItems = [
